                    ...
                    <form action="<?php echo U('news/add');?>" method="post" class="cmxform" enctype="multipart/form-data">
                        <div class="panel">
                            ...
                            <div class="panel-body">
                                    ...
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">分类</span>
                                            <select name="cateid" id="standard-list1" class="form-control">
                                                <option>请选择</option>
                                                <?php if(is_array($category)): foreach($category as $key=>$cate): ?><option <?php echo ($manage['cateid']==$cate['id']?"selected":""); ?> value="<?php echo ($cate["id"]); ?>"><?php echo ($cate["name"]); ?></option><?php endforeach; endif; ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">标题</span>
                                            <input type="text" name="title" value="<?php echo ($manage["title"]); ?>" class="form-control">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">作者</span>
                                            <input type="text" name="author" value="<?php echo ($manage["author"]); ?>" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group col-md-12">
                                      <label for="article-content" class="sr-only">内容</label>
                                      <script id="article-content" name="content" type="text/plain"><?php echo (htmlspecialchars_decode($manage["content"])); ?></script>
                                </div>
                                <div class="form-group col-md-12">
                                ...
                                <div class="col-md-7">
                                    <div class="form-group">
                                        <input type="hidden" value="<?php echo date('Y-m-d H:i:s',time());?>" name="date">
